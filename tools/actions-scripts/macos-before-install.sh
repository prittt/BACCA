...
# Download dataset
echo - e "\n\n------------------------------------------> Download of BACCA reduced dataset (only if it wasn't cached):" 
if [ -d input -a "$(ls -A input/)" ]; then
    echo -e "    BACCA dataset already downloaded"
else
    echo -e "    BACCA dataset not cached, downloading it..."
    wget https://aimagelab.ing.unimore.it/files/YACCLAB_dataset_reduced.zip -O dataset.zip 
