...
bool BACCATests::LoadFileList(vector<pair<string, bool>>& filenames, const path& files_path)
{
...
bool BACCATests::CheckFileList(const path& base_path, vector<pair<string, bool>>& filenames)
{
    bool ret = true;
...
bool BACCATests::SaveBroadOutputResults(map<String, Mat1d>& results, const string& o_filename, const vector<pair<string, bool>>& filenames, const std::vector<AlgorithmNames>& thin_algorithms)
{
...
bool BACCATests::SaveBroadOutputResults(const Mat1d& results, const string& o_filename, const vector<pair<string, bool>>& filenames, const std::vector<AlgorithmNames>& thin_algorithms)
{
    ofstream os(o_filename);
    if (!os.is_open()) {
        return false;
...
void BACCATests::SaveAverageWithStepsResults(const string& os_name, const String& dataset_name, bool rounded)
{
    ofstream os(os_name);
    if (!os.is_open()) {
            ...
            for (unsigned file = 0; file < filenames.size(); ++file) {
                ...
                for (const auto& algo_name : shuffled_thin_average_algorithms) {
                    ...
                    try {
                        ...
                        ob.Cerror("Something wrong with " + algo_name + ": " + e.what()); // You should check your algorithms' implementation before performing BACCA tests  
                    }
...
void BACCATests::AverageTestWithSteps()
{
        ...
        for (unsigned file = 0; file < filenames.size(); ++file) {
            ...
            for (unsigned i = 0; i < cfg_.thin_mem_algorithms.size(); ++i) {
                ...
                try {
                    ...
                    algorithm->FreeChainCodeData();
                    ob.Cerror("Something wrong with " + cfg_.thin_mem_algorithms[i].test_name + ": " + e.what()); // You should check your algorithms' implementation before performing BACCA tests  
                }
...
void BACCATests::LatexGenerator()
{
    ...
    // Document begin
    os << "%These file is generated by BACCA. Follow our project on GitHub: https://github.com/prittt/BACCA" << '\n' << '\n';
    ...
    os << "\\title{{ \\huge\\bfseries BACCA TESTS}}" << '\n';
    os << "\\date{" + GetDatetime() + "}" << '\n';
    os << "\\author{}" << '\n' << '\n';
    os << "\\begin{document}" << '\n' << '\n';
    os << "\\maketitle" << '\n' << '\n';
